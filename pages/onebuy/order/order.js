...
Page({
    ...
    getOrder(options) {
        ...
        let {receiver_id, receiver_address, receiver_name, receiver_phone, gid=0} = options;

        let {defaultreceiver, store_info, id_info, total, is_gold_copper, is_silver, ginfo, total_price, is_gold, is_yu} = this.data;
        ...
        app.post('Order/confirmOrder_new', ginfo, (res) => {
            if (res.code == 200) {
                let {defaultreceiver, result, user_account} = res.data;
                ...
                if (user_account.surplus_goldshells > 0) {
                    total.payment = 0
                } else {
                    total.payment = 100
                }
    ...
    openPageAddress: function () {
        ...
        app.openPage('mine/adress/adress?history_page=order&type=one')
    ...
    pay: function () {
        ...
        wx.showLoading();
        let {defaultreceiver, result, id_info, ginfo, gid} = this.data;
        ...
        function pay_success(order_id, flag) {
            ...
            app.openPage(`onebuy/pay/pay?types=支付&status=200&btn=3,2&order_id=${order_id}&business_id=${result[0].business_id}&ordertype=dtake&buy_type=online&gid=${gid}`, `close`);
        }

        // 支付失败的情况
        function pay_error(order_id) {
            app.post('Order/payFail', {});
            app.openPage(`onebuy/pay/pay?types=支付&status=0&btn=3,2&order_id=${order_id}&business_id=${result[0].business_id}&ordertype=dpay&buy_type=online&gid=${gid}`, `close`);
        }
